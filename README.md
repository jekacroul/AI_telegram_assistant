...
- **Telegram admin-панель**. Полное управление с телефона: меню с кнопками
  дублирует всю веб-панель — статус, настройки, обучение, статистика, стиль,
  репликация, бэкапы, RAG, календарь и заготовки.
...
нажми «Определить автоматически» в **Settings → Admin Panel** или задай
его прямо из бота: **Настройки → 🔔 Уведомления → 🆔 Задать owner chat_id**.

## Полное управление с телефона

Весь функционал веб-панели доступен прямо в Telegram — `/start` открывает
меню с инлайн-кнопками. Разделы:

- **📊 Статус / 🖥 Ресурсы** — здоровье LLM/бота/БД, нагрузка RAM/CPU/VRAM.
- **📋 Очередь** — сообщения на ручной ответ, генерация вариантов, отзывы.
- **⚙️ Настройки** — авто-ответ, расписание, фильтр качества, задержка,
  режим персоны, ответы в группах, авто-очистка очереди, саммари,
  температуры, тайминги, слова-триггеры встреч, уведомления.
- **📈 Статистика** — обзор, активность, топ чатов, время ответа,
  качество моделей, голосовые сообщения.
- **🧠 Обучение** — сборка датасета, запуск/отмена обучения, история
  запусков, активация адаптеров, Llama-server, выбор LLM-модели, Whisper.
- **🎭 Стиль** — глобальный профиль стиля и персоны по чатам.
- **💾 Данные** — репликация БД, бэкап диалогов (с выгрузкой файлом),
  RAG-память (индексация и семантический поиск).
- **📅 Календарь** — события, свободные слоты, создание событий,
  настройки CalDAV.
- **⚡ Заготовки** — быстрые ответы (создание/удаление).

Команды-ярлыки: `/status`, `/pending`, `/settings`, `/stats`, `/training`,
`/replication`, `/rag`, `/dialogs`, `/quick`, `/style`, `/calendar`,
`/pause`, `/resume`, `/help`.
