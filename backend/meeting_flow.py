...
def format_recent_actions(actions: list[dict]) -> Optional[str]:
    ...
    lines.append(
        ...
        " Категорически нельзя: говорить что ты в это время занят или"
        " не сможешь; переспрашивать время; предлагать другой вариант;"
        " противоречить только что выполненной записи. Действие уже"
        " совершено — менять решение нельзя."
    )
    return "\n".join(lines)
